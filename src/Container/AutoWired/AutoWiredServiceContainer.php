...
namespace RahimiAli\Hermes\Container\AutoWired;

use InvalidArgumentException;
use Psr\Container\ContainerExceptionInterface;
use Psr\Container\NotFoundExceptionInterface;
use RahimiAli\Hermes\Container\AutoWired\Exceptions\CircularDependencyException;
use RahimiAli\Hermes\Container\AutoWired\Exceptions\FailedToResolveParameterException;
use RahimiAli\Hermes\Container\AutoWired\Exceptions\NotFoundException;
use RahimiAli\Hermes\Container\AutoWired\Exceptions\UninstantiableClassException;
use RahimiAli\Hermes\Container\AutoWired\Utils\InteractsWithReflection;
use RahimiAli\Hermes\Container\AutoWired\Utils\NotResolved;
use RahimiAli\Hermes\Container\ConfigurableServiceContainer;
...
class AutoWiredServiceContainer implements ConfigurableServiceContainer
{
    ...
    /**
     * @var array<string, array{bindingType: ServiceContainer::BINDING_TYPE_*, callback: callable(ServiceContainer $container): mixed}>
     */
    ...
    public function makeOnce(string $className): static
    {
        $this->bindOnce(
            $className,
            static fn (ServiceContainer $container) => $container->make($className)
        );
        return $this;
    }
     ...
     * @return string[]
     */
    private function expandId(string $id): array
    {
        return interface_exists($id) ? self::extendInterface($this->getReflectionClass($id)) : [$id];
    }
    ...
    private function makeParameter(ReflectionParameter $parameter, array $parameters, string $context = ''): mixed
    {
        ...
        if ($paramType !== null) {
            ...
            foreach ($types as $subType) {
                $resolved = $this->resolve($subType);
                if (!$resolved instanceof NotResolved) {
                    return $resolved;
                }
                try {
                    /** @phpstan-ignore-next-line */
                    return $this->make($subType);
